...
def getAllNetworks():
    ...
    for iface in ifaces:
        ...
        netmask = str(sum([bin(int(x)).count('1')
                           for x in iNetwork.getAdapterAttribute(iface, "netmask")]))
        ip = ip + "/" + netmask
...
def isOriginalWebifInstalled():
    pluginpath = enigma.eEnv.resolve(
        '${libdir}/enigma2/python/Plugins/Extensions/WebInterface/plugin.py')
    if fileExists(pluginpath) or fileExists(
            pluginpath +
            "o") or fileExists(
            pluginpath +
            "c"):
...
def buildRootTree(session):
    ...
    if not isOriginalWebifInstalled():
        ...
        origwebifpath = enigma.eEnv.resolve(
            '${libdir}/enigma2/python/Plugins/Extensions/WebInterface')
        hookpath = enigma.eEnv.resolve(
...
# print "[OpenWebif] plugin '%s' loaded on path '/%s'" % (plugin[2],
# plugin[0])
        else:
...
class AuthResource(resource.Resource):
    ...
    def noShell(self, request):
        ...
        if fileExists('/etc/passwd'):
            for line in file('/etc/passwd').readlines():
                line = line.strip()
                if line.startswith(
                        user + ":") and line.endswith(":/bin/false"):
                    return True
        return False
    ...
    def render(self, request):
        ...
        if (host == "localhost" or host == "127.0.0.1" or host ==
                "::ffff:127.0.0.1") and not config.OpenWebif.auth_for_streaming.value:
    ...
    def getChildWithDefault(self, path, request):
        ...
        if (not request.isSecure() and config.OpenWebif.auth.value == False) or (
                request.isSecure() and config.OpenWebif.https_auth.value == False):
            networks = getAllNetworks()
            if networks is not None:
                for network in networks:
                    if ipaddress.ip_address(
                            unicode(peer)) in ipaddress.ip_network(
                            unicode(network), strict=False):
                        return self.resource.getChildWithDefault(path, request)

        # #2: Auth is disabled and access is from private address space (Usually VPN) and access for VPNs has been granted
        if (not request.isSecure() and config.OpenWebif.auth.value == False) or (
                request.isSecure() and config.OpenWebif.https_auth.value == False):
            if config.OpenWebif.vpn_access.value and ipaddress.ip_address(
                    unicode(peer)).is_private:
                return self.resource.getChildWithDefault(path, request)

        # #3: Access is from localhost and streaming auth is disabled - or - we only want to see our IPv6 (For inadyn-mt)
        if ((host == "localhost" or host == "127.0.0.1" or host == "::ffff:127.0.0.1" or host == "::1")
                and not config.OpenWebif.auth_for_streaming.value) or request.uri == "/web/getipv6":
            return self.resource.getChildWithDefault(path, request)
        ...
        if (not request.isSecure() and config.OpenWebif.auth.value == False) or (
                request.isSecure() and config.OpenWebif.https_auth.value == False):
            errpage = resource.ErrorPage(
                http.FORBIDDEN, 'Forbidden', '403.6 IP address rejected')
            return errpage
