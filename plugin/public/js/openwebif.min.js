...
$(function(){SetSpinner();try{$("#dialog").dialog({autoOpen:!1,show:"fade",hide:"explode",buttons:{Ok:function(){$(this).dialog("close")}}}),$("#volimage").click(function(){0===mutestatus?(mutestatus=1,$("#volimage").attr("src","/images/volume_mute.png")):(mutestatus=0,$("#volimage").attr("src","/images/volume.png"));$.ajax("web/vol?set=mute")}),getStatusInfo(),setInterval("getStatusInfo()",15E3),$("#slider").slider({range:"min",min:0,max:100,value:40,slide:function(a,c){$("#amount").val(c.value)},
stop:function(a,c){0==c.value?$("#volimage").attr("src","/images/volume_mute.png"):$("#volimage").attr("src","/images/volume.png");$.ajax("web/vol?set=set"+c.value);return!1}}),$("#amount").val($("#slider").slider("value")),$(".epgsearch button:first").button({icons:{primary:"ui-icon-search"}}),_locations=loadLocations(),_tags=loadTags()}catch(a){}});
...
function getStatusInfo(){$.ajax({url:"/api/statusinfo",dataType:"json",cache:!1,success:function(a){$("#slider").slider("value",a.volume);$("#amount").val(a.volume);!0==a.muted?(mutestatus=1,$("#volimage").attr("src","/images/volume_mute.png")):(mutestatus=0,$("#volimage").attr("src","/images/volume.png"));setOSD(a);var b="";"true"==a.isRecording&&(b="<a href='#' onClick='load_maincontent(\"ajax/timers\"); return false;'><div title='"+tstr_rec_status+a.Recording_list+"' class='led-box'><div class='led-red'></div></div></a>");
b+="<a href='#' onClick='toggleStandby();return false'><img src='../images/ico_";b="true"==a.inStandby?b+("standby.png' title='"+tstr_on+"' alt='"+tstr_standby):b+("on.png' title='"+tstr_standby+"' alt='"+tstr_on);$("#osd_status").html(b+"' width='56' height='29' /></a>")},error:function(){$("#osd, #osd_bottom").html("")}})}
