...
class BouquetEditor(Source):
    ...
    def addBouquet(self, bName, mode, services):
        if config.usage.multibouquet.value:
            ...
            if mutableBouquetList:
                if mode == MODE_TV:
                    bName += " (TV)"
                    sref = '1:7:1:0:0:0:0:0:0:0:FROM BOUQUET \"userbouquet.%s.tv\" ORDER BY bouquet' % (
                        self.buildBouquetID(bName, "userbouquet.", mode))
                else:
                    bName += " (Radio)"
                    sref = '1:7:2:0:0:0:0:0:0:0:FROM BOUQUET \"userbouquet.%s.radio\" ORDER BY bouquet' % (
                        self.buildBouquetID(bName, "userbouquet.", mode))
                ...
                if not mutableBouquetList.addService(new_bouquet_ref):
                    ...
                    if mutableBouquet:
                        ...
                        return (
                            False, _("Get mutable list for new created bouquet failed!"))
    ...
    def addProviderToBouquetlist(self, param):
        ...
        return self.addBouquet(
            providerName,
            mode,
            services and services.getContent(
                'R',
                True))
    ...
    def removeBouquet(self, param):
        ...
        if ref.valid() and mutableList is not None:
            ...
            return (
                False,
                _("Bouquet %s removed failed, sevicerefence or mutable list is not valid.") %
                filename)
        try:
            ...
            return (
                False,
                _("Error: Bouquet %s could not deleted, OSError.") %
                filename)
    ...
    def removeService(self, param):
        ...
        ref = eServiceReference(sRef)
        # check if  service is an alternative, if so delete it with
        # removeBouquet
        if ref.flags & eServiceReference.isGroup:
            new_param = {}
            new_param["sBouquetRef"] = sRef
            # of no interest when passing BouquetRefRoot
            new_param["mode"] = None
            ...
            if mutableBouquetList is not None:
                if not mutableBouquetList.removeService(ref):
                    ...
                    return (
                        True, _("Service %s removed from bouquet %s.") %
                        (self.getName(ref), self.getName(bouquetRef)))
    ...
    def addServiceToBouquet(self, param):
        ...
        if mutableBouquetList is not None:
            ...
            if not mutableBouquetList.addService(ref, sRefBefore):
                ...
                return (
                    False, _("Service %s already exists in bouquet %s.") %
                    (self.getName(ref), bouquetName))
    ...
    def renameService(self, param):
        ...
        if cur_ref.flags & eServiceReference.mustDescent:
            ...
            # services can not be renamed directly, so delete the current and
            # add it again with new servicename
            sRefBefore = None
            if "sRefBefore" in param:
                if param["sRefBefore"] is not None:
    ...
    def addServiceToAlternative(self, param):
        ...
        if not (cur_ref.flags & eServiceReference.isGroup):
            # sCurrentRef is not an alternative service yet, so do this and add
            # itself to new alternative liste
            mode = MODE_TV  # init
            if "mode" in param:
                if param["mode"] is not None:
                    mode = int(param["mode"])
            ...
            if mutableBouquetList:
                ...
                if not mutableBouquetList.addService(new_ref, cur_ref):
                    ...
                    if mutableAlternatives:
                        ...
                        self.setRoot(sBouquetRef)
                        # currentRef is now an alternative (bouquet)
                        sCurrentRef = sref
                    else:
                        return (
                            False, _("Get mutable list for new created alternative failed!"))
                else:
        ...
        if returnValue[0]:
            ...
            return (
                True, _("Added %s to alternative service %s.") %
                (service_name, name))
        else:
            return returnValue
        ...
        if cur_ref.flags & eServiceReference.isGroup:
            ...
            if first_in_alternative:
                mutableBouquetList = self.getMutableList(
                    eServiceReference(sBouquetRef))
                if mutableBouquetList is not None:
                    if mutableBouquetList.addService(
                            first_in_alternative, cur_service.ref):
    ...
    def toggleLock(self, param):
        ...
        if protection:
            if config.ParentalControl.type.value == "blacklist":
                ...
                if hasattr(
                        ParentalControl,
                        "whitelist") and sref in parentalControl.whitelist:
    ...
    def backupFiles(self, param):
        ...
        if f:
            ...
            files += self.getPhysicalFilenamesFromServicereference(eServiceReference(
            ...
            for arg in files:
                if not path.exists(arg):
                    return (
                        False,
                        _("Error while preparing backup file, %s does not exists.") %
                        arg)
                tarFiles += "%s " % arg
            lines = popen(
                "tar cvf %s %s" %
                (backupFilename, tarFiles)).readlines()  # nosec
            remove(checkfile)
    ...
    def restoreFiles(self, param):
        tarFilename = param
        # path.join(self.BACKUP_PATH, tarFilename)
            ...
            if check_tar:
                ...
                for bouquetfiles in files:
                    if path.exists(bouquetfiles):
                        remove(bouquetfiles)
                lines = popen(
                    'tar xvf %s -C / --exclude tmp/.webouquetedit' %
                    backupFilename).readlines()  # nosec
