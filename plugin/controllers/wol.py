        ...
        if len(request.args):
            ...
            if "wol" in request.args:
                ...
                if wol_standby in ('true', '1', 'enabled', 'enable', 'active'):
                    ...
                    self.session.open(
                        TryQuitMainloop, _tryQuitTable["deepstandby"])
            if config_changed:
    ...
    def render(self, request):
        ...
        if len(request.args):
            ...
            if ip and mac:
                ...
                my_socket.close()
                return """<?xml version=\"1.0\" encoding=\"UTF-8\" ?><e2simplexmlresult><e2state>true</e2state><e2statetext>MagicPacket send to IP %s at port %d</e2statetext></e2simplexmlresult> """ % (
                    ip, port)
            else:
