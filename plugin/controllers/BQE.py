...
<e2simplexmlresult>
    ...
    def P_addbouquet(self, request):
        ...
        try:
            ...
            return self.returnResult(
                request, [False, 'BouquetEditor plugin not found'])
    ...
    def P_removebouquet(self, request):
        ...
        try:
            ...
            bqe = BouquetEditor(
                self.session,
                func=BouquetEditor.REMOVE_BOUQUET)
            bqe.handleCommand(
                self.buildCommand(
                    'sBouquetRef,mode',
                    request.args))
    ...
    def P_addmarkertobouquet(self, request):
        ...
        try:
            ...
            bqe = BouquetEditor(self.session,
                                func=BouquetEditor.ADD_MARKER_TO_BOUQUET)
            bqe.handleCommand(
                self.buildCommand(
                    'sBouquetRef,Name,sRefBefore,SP',
                    request.args))
            return self.returnResult(request, bqe.result)
        except ImportError:
            return self.returnResult(
    ...
    def P_addservicetobouquet(self, request):
        ...
        try:
            from BouquetEditor import BouquetEditor
            bqe = BouquetEditor(self.session,
                                func=BouquetEditor.ADD_SERVICE_TO_BOUQUET)
            bqe.handleCommand(
                self.buildCommand(
                    'sBouquetRef,sRef,sRefBefore',
                    request.args))
            return self.returnResult(request, bqe.result)
        except ImportError:
            return self.returnResult(
    ...
    def P_addprovidertobouquetlist(self, request):
        ...
        try:
            from BouquetEditor import BouquetEditor
            bqe = BouquetEditor(self.session,
                                func=BouquetEditor.ADD_PROVIDER_TO_BOUQUETLIST)
            bqe.handleCommand(
                self.buildCommand(
                    'sProviderRef,mode',
                    request.args))
    ...
    def P_addservicetoalternative(self, request):
        ...
        try:
            ...
            bqe.handleCommand(
                self.buildCommand(
                    'sBouquetRef,sCurrentRef,sRef,mode',
                    request.args))
            return self.returnResult(request, bqe.result)
        except ImportError:
            return self.returnResult(
    ...
    def P_moveservice(self, request):
        ...
        try:
            ...
            bqe.handleCommand(
                self.buildCommand(
                    'sBouquetRef,sRef,mode,position',
                    request.args))
            return self.returnResult(request, bqe.result)
        except ImportError:
            return self.returnResult(
    ...
    def P_removeservice(self, request):
        ...
        try:
            ...
            bqe = BouquetEditor(
                self.session,
                func=BouquetEditor.REMOVE_SERVICE)
            bqe.handleCommand(
                self.buildCommand(
                    'sBouquetRef,sRef',
                    request.args))
    ...
    def P_renameservice(self, request):
        ...
        try:
            ...
            bqe.handleCommand(
                self.buildCommand(
                    'sBouquetRef,sRef,sRefBefore,newName,mode',
                    request.args))
            return self.returnResult(request, bqe.result)
        except ImportError:
            return self.returnResult(
    ...
    def P_calcpos(self, request):
        ...
        bRef = '%s FROM BOUQUET "bouquets.tv" ORDER BY bouquet' % (
            service_types_tv)
        if type == 1:
            bRef = '%s FROM BOUQUET "bouquets.radio" ORDER BY bouquet' % (
                service_types_radio)
    ...
    def P_getprotectionsettings(self, request):
        ...
        if configured:
            if config.ParentalControl.type.value == "blacklist":
                ...
                type = "1"
            setuppin = "setuppin" in config.ParentalControl.dict(
            ).keys() and config.ParentalControl.setuppin.value or -1
            setuppinactive = "setuppin" in config.ParentalControl.dict(
            ).keys() and config.ParentalControl.setuppinactive.value
        else:
            type = ""
            setuppin = ""
            setuppinactive = ""
